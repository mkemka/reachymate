...
class Camera(Tool):
    ...
    async def __call__(self, deps: ToolDependencies, **kwargs: Any) -> Dict[str, Any]:
        ...
        h, w = frame.shape[:2]
        logger.info("camera: frame %dx%d (%d bytes raw)", w, h, frame.nbytes)
        success, buffer = cv2.imencode('.jpg', frame, [cv2.IMWRITE_JPEG_QUALITY, 98])
        if not success:
            raise RuntimeError("Failed to encode frame as JPEG")

        b64_encoded = base64.b64encode(buffer.tobytes()).decode("utf-8")
        logger.info("camera: JPEG %d bytes, base64 %d chars", len(buffer), len(b64_encoded))
